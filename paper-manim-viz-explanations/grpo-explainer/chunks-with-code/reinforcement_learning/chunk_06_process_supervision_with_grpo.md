...
**Explanation:** The paper chunk describes process supervision with GRPO, including reward calculation at each reasoning step, reward normalization, and advantage calculation. The GRPOTrainer implementation directly handles these concepts through reward processing, normalization, and advantage computation methods.

### Code Section 1: Method for calculating rewards in GRPO training, which implements the process supervision reward calculation described in the paper. This includes computing rewards for each step of the outputs and normalizing them.
...
### Code Section 2: The reward_func method demonstrates how rewards are computed for completions, which relates to the process supervision concept of providing rewards at each reasoning step.

**File:** `grpo_trainer.py` (lines 131-237)
...
### Code Section 3: The _compute_loss method implements the GRPO objective function and advantage calculation. It computes advantages as sums of future rewards, matching the formula Â_{i,t} = Σ_{index(j) ≥ t} ṟ_i^{index(j)} from the paper.
...
### Code Section 4: The GRPOConfig class defines training parameters that control the process supervision behavior, including reward scaling and importance sampling strategies mentioned in the paper.
