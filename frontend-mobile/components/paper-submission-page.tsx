...
export function PaperSubmissionPage() {
  ...
  const handleSubmit = async () => {
    ...
    // Wait 2 seconds before loading content
        ...
        <Card className="mb-6">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="arxiv-url" className="flex items-center gap-2 text-base">
            ...
            {/* File Upload */}
            <div className="space-y-2">
              <Label htmlFor="pdf-upload" className="flex items-center gap-2 text-base">
                <Upload className="h-4 w-4" />
                Upload PDF
              </Label>
              <div
                className="cursor-pointer rounded-lg border-2 border-dashed border-input p-6 text-center transition-colors active:border-primary touch-manipulation"
        ...
        <Card className="mb-6">
          <CardHeader className="pb-3">
            <CardTitle className="flex items-center gap-2 text-lg">
              ...
              Videos
              {videos.length > 0 && (
                <span className="ml-2 rounded-full bg-primary/10 px-2.5 py-1 text-xs font-normal text-primary">
                  {videos.length}
                </span>
              )}
            </CardTitle>
                ...
                <div className="flex h-64 items-center justify-center text-muted-foreground">
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <div className="text-center space-y-1">
                      ...
                      <p className="text-xs text-muted-foreground">Generating videos</p>
                    </div>
...
function VideoCard({ video }: { video: VideoItem }) {
  ...
  return (
    <>
      <div className="flex flex-col gap-3 rounded-lg border bg-card p-3 transition-colors active:bg-muted/50 touch-manipulation">
        <div className="flex gap-3">
          ...
          <div className="flex flex-col justify-center overflow-hidden flex-1 min-w-0">
            ...
            <h3 className="font-semibold text-foreground mb-1 text-sm line-clamp-2">{video.title}</h3>
            <p className="line-clamp-2 text-xs text-muted-foreground mb-2">{video.description}</p>
            <Button
              variant="outline"
              size="sm"
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-[95vw] max-h-[95vh] flex flex-col p-4">
          <DialogHeader>
          ...
          <div className="flex flex-col gap-4 flex-1 min-h-0">
            ...
            <div className="w-full h-48 sm:h-64 bg-muted rounded-lg overflow-hidden flex-shrink-0">
              <video
                src={video.videoPath}
                controls
...
function MarkdownContent({ content }: { content: string }) {
  const renderLine = (line: string, index: number) => {
    ...
    if (line.startsWith('# ')) {
      return <h1 key={index} className="text-xl font-bold mt-4 mb-2">{renderInlineMath(line.substring(2))}</h1>
    } else if (line.startsWith('## ')) {
      return <h2 key={index} className="text-lg font-bold mt-3 mb-2">{renderInlineMath(line.substring(3))}</h2>
    } else if (line.startsWith('### ')) {
      return <h3 key={index} className="text-base font-semibold mt-2 mb-1">{renderInlineMath(line.substring(4))}</h3>
    } else if (line.trim() === '') {
      return <br key={index} />
    }
